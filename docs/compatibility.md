...
| 2026-06-02 | CPython 3.13.13 | unreleased data lake hardening | `xtquant_250516` from local venv | `userdata_mini` | market data lake download, Parquet cache, coverage, bars query, quality, CSV export, cached download | Readonly smoke `uv run python scripts\smoke_market_data_lake.py --symbol 000001.SZ --require-rows` passed at 23:51 local time. The smoke verified `download_history_data` followed by local Parquet write, `fully_covered=true`, 6 queried rows, 6 exported rows, and a second download returning `cached=true`. `trader_connected=false`; no trader or trading commands used. |
| 2026-06-03 | CPython 3.13.13 | unreleased data lake query hardening | `xtquant_250516` from local venv | `userdata_mini` | market data lake DuckDB query path and default completed-day smoke window | Readonly smoke `uv run python scripts\smoke_market_data_lake.py --symbol 000001.SZ --require-rows` passed at 00:36 local time after switching the default daily window to end on the previous day. It verified the DuckDB-backed local bars/export path, 6 queried rows, 6 exported rows, complete coverage, and cached second download. `trader_connected=false`; no trader or trading commands used. |
| 2026-06-06 | CPython 3.13.13 | 0.9.0 release candidate | local `xtquant` install | `userdata_mini` | market data lake download, Parquet cache, coverage, bars query, quality, CSV export, cached download, per-symbol result | Readonly smoke `uv run python scripts\smoke_market_data_lake.py --symbol 000001.SZ --require-rows --timeout-seconds 60` passed at 16:50 local time. It verified xtdata connection, one successful download job, `fully_covered=true`, 6 queried rows, 6 exported rows, per-symbol `symbol_results`, and a second download returning `cached=true`. `trader_connected=false`; no trader or trading commands used. |
...
| `scripts/smoke_market_data_lake.py` | qmtserver smoke helper | Checks `/v1/health` and `/v1/market/data/*` download, coverage, bars, quality, export, cached download, and per-symbol summaries with `connect_trader=False` | `tests/test_smoke_market_data_lake_script.py`, `tests/test_api_market_data.py`, `tests/test_market_normalizers.py`, `tests/test_data_backend.py` | 2026-06-06 readonly smoke passed for `000001.SZ` with non-empty rows, Parquet cache, coverage, export, cached second download, and `symbol_results` | development verified |
