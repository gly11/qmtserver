...
| `scripts/smoke_market_data_lake.py` | qmtserver smoke helper | Checks `/v1/health` and `/v1/market/data/*` download, coverage, bars, quality, export, and cached download paths with `connect_trader=False` | `tests/test_smoke_market_data_lake_script.py`, `tests/test_api_market_data.py`, `tests/test_market_normalizers.py`, `tests/test_data_backend.py` | 2026-06-02 readonly smoke passed for `000001.SZ` with non-empty rows, Parquet cache, coverage, export, and cached second download | development verified |
